	...
	<head>
		...
		<script >
			so.init(function(){
				...
				<@shiro.hasPermission name="/permission/deletePermissionById.shtmlno">
				//全选
			...
			<@shiro.hasPermission name="/permission/addPermission.shtmlno">
			<#--添加权限-->
			function addPermission(){
				...
				if($.trim(method) == ''){
					return layer.msg('权限请求方式不能为空。',so.default),!1;
				}
				<#--loding-->
				var load = layer.load();
				$.post('${basePath}/permission/addPermission.shtml',{name:name,url:url,method:method},function(result){
					layer.close(load);
					if(result && result.status != 200){
			      ...
			      <div class="modal-body">
			        <form id="boxRoleForm">
			          ...
			          <div class="form-group">
			            ...
			            <input type="text" class="form-control" id="method" name="method"  placeholder="请输入权限的请求方式">
			          </div>
			        </form>
			      </div>
